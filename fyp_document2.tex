...
One main issue I found with the Gridstack library is that if a chart contains no data it does not scale the chart correctly and makes the chart overflow to one side making it appear compact. Unfortunately this issue is not consistent and as a result was not fixed. Currently the only fix is to click on the resizeable handles of a widget and the chart then scales correctly.
\paragraph{Billboard.js}
%talk about how billboard was used for the dashboard charting 
The chart type support will be discussed in \cref{para:charts:fontend}.
\paragraph{Vue.js}
